...
let cfg = null;
try { const _t = document.querySelector('.brand .tag'); if (_t) { _t.textContent = 'Panel v' + PANEL_VERSION; _t.title = 'Marna dashboard v' + PANEL_VERSION; } console.log('%cMarna panel v' + PANEL_VERSION, 'color:#22d3ee;font-weight:700'); } catch (e) {}
...
<p style="font-size:13px">On <b>Subscriptions</b>, the <b>Free-service notice</b> is <b>always on</b> and its <b>message is fixed</b> — a labelled node is baked into every config so people always see it's the free Marna service. It can't be turned off or changed by anyone (so resellers can't strip or rebrand it).</p>
</div>
...
async function loadAll(){
  ...
  // Default the Smart IP API to Marna's shared per-ISP database so users get it out of the box.
...
function marna5(){ const a='abcdefghijklmnopqrstuvwxyz0123456789'; let s=''; for(let i=0;i<5;i++) s+=a[Math.floor(Math.random()*36)]; return s; }
...
async function startScan(onProg){
  ...
  if(src==='random'){ while(ips.length<total){ const ip=randCfIp(); if(!seen.has(ip)){ seen.add(ip); ips.push(ip); } } }
    ...
    // Pull candidate IPs from a Marna radar source, then latency-test them in the browser.
...
if($('sc-use')) $('sc-use').onclick=async()=>{
  ...
  const lines=scanResults.map(r=>r.ip+':'+r.port+'#Marna-'+marna5());
...
(function(){
  try{
    ...
    function show(){
      ...
      const f=fa();
      const txt = isIOS
        ? (f?'نصب نوا روی صفحه اصلی: دکمهٔ اشتراک‌گذاری سافاری (⬆️) ← «Add to Home Screen»':'Install Marna: Safari Share (⬆️) → “Add to Home Screen”')
        : (f?'نصب «نوا پروکسی» روی گوشی':'Install Marna Proxy on your phone');
